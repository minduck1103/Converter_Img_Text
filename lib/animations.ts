...
export const FadeLeft = (delay: number = 0) => {
  return {
    hidden: {
      opacity: 0,
      x: 100,
...
export const FadeRight = (delay: number = 0) => {
  return {
    hidden: {
      opacity: 0,
      x: -100,
...
export const slideIn = (direction: 'left' | 'right' | 'up' | 'down', type: string, delay: number = 0, duration: number = 0.5) => {
  return {
    hidden: {
